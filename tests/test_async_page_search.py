...
class TestGetClimbDetails(asynctest.TestCase):
    ...
    def test_climb_name(self):
        ...
        result = self.loop.run_until_complete(async_page_search.climb_name(request_text))

        self.assertEqual('Yellow Zonkers', result)

    def test_grade_finder(self):
        """Testing the the grade gets found and returned correctly"""
        result = self.loop.run_until_complete(async_page_search.grade_finder(request_text))
    ...
    def test_navigation_tree(self):
        ...
        result = self.loop.run_until_complete(async_page_search.navigation_tree(request_text))

        self.assertEqual(['pawtuckaway', 'upper-cliff'], result)

    def test_list_maker(self):
        """Integration test for the list maker function"""
        result = self.loop.run_until_complete(async_page_search.list_maker('https://www.mountainproject.com/route/107235878/yellow-zonkers',
                                                                           request_text,
                                                                           'ow'))
